...
def show_spider_emotion():
    data = [(p.name,list(p.get_emotional_stats().values())) for p in SIMPLE_PEEPS]
    labels = list(SIMPLE_PEEPS[0].get_emotional_stats().keys())
...
def show_spider_physical():
    data = [(p.name,list(p.get_physical_stats().values())) for p in SIMPLE_PEEPS]
    labels = list(SIMPLE_PEEPS[0].get_physical_stats().keys())
...
def show_spider_all():
    data = [(p.name, list(p.stat_apts.values())) for p in SIMPLE_PEEPS]
    labels = list(SIMPLE_PEEPS[0].stat_apts.keys())
...
def show_spider_specific(peep_name: str):
    ...
    # specified peep
    peep: Character = [p for p in SIMPLE_PEEPS if p.name == peep_name][0]
